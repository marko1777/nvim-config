return {
	{ "github/copilot.vim" },
	{
		...
		branch = "main",
		keys = {
			{ "<leader>cc", "<cmd>CopilotChat<CR>", desc = "[c]o[p]ilot [c]hat", mode = { "n", "x" } },
			{ "<leader>cr", "<cmd>CopilotChatReview<CR>", desc = "[c]o[p]ilot [r]eview", mode = { "n", "x" } },
			{
				"<leader>cgt",
				"<cmd>CopilotChatGoTests<CR>",
				desc = "[c]o[p]ilot generate [g]o [t]est",
				mode = { "n", "x" },
			},
			{
				"<leader>capi",
		...
		opts = {
			...
			prompts = {
				...
				GoTests = "Please generate Go unit tests for the following code. Use testify for assertions with require. Use table-driven tests. Every case should be run in parallel. Please generate a case for every possible input.",
